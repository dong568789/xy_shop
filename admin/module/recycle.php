...
switch ($act) {
	...
	break;
		...
		$order_id = pe_dbhold($_g_id);
		$info = $db->pe_select('order', array('order_id'=>$order_id));
		if ($info['order_state'] != 'recycle') pe_error('还未申请回购...', '', 'dialog');
	
		if (isset($_p_pesubmit) || (isset($_p_order_wl_id) && $_p_order_wl_id > 0)) {
		    
			pe_token_match();
		    $result = order_callback_agree($info,$_p_order_wl_id);
			if ($result) {
				pe_success('审核成功!', '', 'dialog');
			}
			else {
				pe_error('审核失败!', '', 'dialog');
			}
		}
		$wlname_list = $cache_setting['web_wlname'] ? explode(',', $cache_setting['web_wlname']) : array();
		//检测是否有退款子订单
		$refund_list = $db->pe_selectall('orderdata', array('order_id'=>$order_id, 'refund_state'=>array('wcheck', 'wsend', 'wget', 'refuse')));
		include(pe_tpl('recycle_send.html','admin'));
	break;
		case 'fail_recycle':
		$order_id = pe_dbhold($_g_id);
		$info = $db->pe_select('order', array('order_id'=>$order_id));
		...
		if (isset($_p_pesubmit)) {
			pe_token_match();
			 $result = order_callback_fail($info);
			if ($result) {
				pe_success('处理成功!', '', 'dialog');
			}
			else {
				pe_error('处理失败!', '', 'dialog');
			}
		}
	//	$wlname_list = $cache_setting['web_wlname'] ? explode(',', $cache_setting['web_wlname']) : array();
		//检测是否有退款子订单
	...
	default:
	    $_g_state = "recycle";
		$_g_state && $sql_where .= " and `order_state` in ('recycle','success','fail')";	
...
// 		}
		...
		if ($act == 'excel_out') {
			...
			include(pe_tpl('recycle_list.html','admin'));
		}
	break;
}
?>