...
switch ($act) {
	...
	//case 'add_pintuan':
		...
		if (isset($_p_pesubmit)) {
			...
			if (!user_checkguest()) pe_jsonshow(array('result'=>false, 'show'=>'Iltimos, avval tizimga kiring'));
 			if (!count($info_list)) pe_jsonshow(array('result'=>false, 'show'=>'Savatdagi buyumlar boʻsh'));
			if (!$_p_order_payment) pe_jsonshow(array('result'=>false, 'show'=>'Toʻlov usulini tanlang'));
			...
			if ($db->pe_insert('order', pe_dbhold($sql_order))) {
				...
				if ($sql_order['order_money'] == 0) {
					...
					pe_jsonshow(array('result'=>true, 'show'=>'Buyurtma toʻlovi muvaffaqiyatli amalga oshirildi', 'url'=>$ret['url']));
				}
				else {
					pe_jsonshow(array('result'=>true, 'show'=>'Buyurtma muvaffaqiyatli topshirildi', 'url'=>"{$pe['host_root']}index.php?mod=order&act=pay&id={$order_id}"));				
				}
			}
			else {
				pe_jsonshow(array('result'=>false, 'show'=>'Buyurtma topshirilmadi'));
			}	
		}
		$seo = pe_seo($menutitle='Buyurtma berish');
	...
	case 'pay':
		...
		$user['user_money'] = pe_num($user['user_money'], 'round', 1, true);
		if (isset($_p_pesubmit)) {
			if (!$info['order_id']) pe_jsonshow(array('result'=>false, 'show'=>"Buyurtma raqami noto'g'ri"));
			if ($info['order_pstate']) pe_jsonshow(array('result'=>false, 'show'=>"Buyurtma to'lanadi"));
			...
			if ($order_payment == 'cod' or !array_key_exists($order_payment, $cache_payment)) pe_jsonshow(array('result'=>false, 'show'=>"Noto'g'ri to'lov usuli"));
			if ($order_payment == 'balance') {
				...
				if ($user['user_money'] < $info['order_money']) pe_jsonshow(array('result'=>false, 'show'=>'Hisob balansi yetarli emas'));
		...
		if (!$info['order_id']) pe_404("Buyurtma raqami noto'g'ri");
		$seo = pe_seo($menutitle="Buyurtmani to'lash");
