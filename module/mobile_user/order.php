...
switch($act) {
	...
	case 'view':
		...
		if($info['order_atime'] > 0){
		    $info['order_atime'] = $info['order_atime'];
		}
		if($info['order_ptime'] > 0){
		    $info['order_ptime'] = $info['order_ptime'];
		}
		if($info['order_stime'] > 0){
		    $info['order_stime'] = $info['order_stime'];
		}
		if($info['order_ftime'] > 0){
		    $info['order_ftime'] = $info['order_ftime'];
		}
		...
		$seo = pe_seo($menutitle='Buyurtma tafsilotlari');
	...
	case 'recycle':
		...
		if (!$info['order_id']) pe_jsonshow(array('result'=>false, 'show'=>'参数错误'));
		if ($info['order_state'] != 'wsend') pe_jsonshow(array('result'=>false, 'show'=>"Buyurtma to'lanmagan"));
		if (in_array($info['order_payment'], array('alipay_db', 'cod'))) pe_jsonshow(array('result'=>false, 'show'=>'参数错误'));
		if (order_callback_recycle($info)) {
			pe_jsonshow(array('result'=>true, 'show'=>'Operatsiya muvaffaqiyatli'));
		}
		else {
			pe_jsonshow(array('result'=>false, 'show'=>'Operatsiya muvaffaqiyatsiz tugadi'));
		}
	break;
	//####################// 订单评价 //####################//
	case 'comment':
	...
	//####################// 订单列表 //####################//
	default:
		if (in_array($_g_state, array('wpay', 'recycle', 'close', 'success'))) {
...
// 		}
		...
		foreach ($info_list as $k => $v){
		    if($v['order_atime'] > 0){
		        $info_list[$k]['order_atime'] = $v['order_atime']-(60*60*3);    
		    }
		     if($v['order_ptime'] > 0){
		        $info_list[$k]['order_ptime'] = $v['order_ptime']-(60*60*3);    
		    }
		    if($v['order_stime'] > 0){
		        $info_list[$k]['order_stime'] = $v['order_stime']-(60*60*3);    
		    }
		    if($v['order_ftime'] > 0){
		        $info_list[$k]['order_ftime'] = $v['order_ftime']-(60*60*3);    
		    }
	...
	//	var_dump($info_list);exit;
		...
		$seo = pe_seo($menutitle='mening buyurtmam');
