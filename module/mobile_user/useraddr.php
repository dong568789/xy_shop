...
// 			if (!$_p_address_text) pe_jsonshow(array('result'=>false, 'show'=>'请填写详细地址'));
		...
		$seo = pe_seo($menutitle="Yangi manzil qo'shing");
	...
	case 'edit':
		...
		$seo = pe_seo($menutitle="Manzilni o'zgartirish");
		include(pe_tpl('useraddr_add.html'));
	break;
	//####################// 地址删除 //####################//
	case 'del':
		...
		if ($db->pe_delete('useraddr', array('address_id'=>$address_id, 'user_id'=>$_s_user_id))) {
			pe_jsonshow(array('result'=>true, 'show'=>'Muvaffaqiyatli oʻchirildi'));
		}
		else {
			pe_jsonshow(array('result'=>false, 'show'=>'Oʻchirish amalga oshmadi'));
		}
	break;
	//####################// 地址列表 //####################//
	default:
		...
		$seo = pe_seo($menutitle='Yetkazib berish manzili');
