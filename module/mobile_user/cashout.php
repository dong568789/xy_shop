...
switch ($act) {
	...
	case 'add':
		...
		if($info['tx'] == 0){
		    ...
		    $reason = $info['reason']?:"Yarımçıq tapşırıqlarınız var və geri çəkilməyiniz rədd edildi";
		    
		   return pe_jsonshow(array('result'=>false, 'show'=>$reason));
		}
			...
			if ($cashout_money < $cache_setting['cashout_min']) pe_jsonshow(array('result'=>false, 'show'=>"Olib tashlash miqdoridan kichik; .. dan kamroq{$cache_setting['cashout_min']} so'm"));
